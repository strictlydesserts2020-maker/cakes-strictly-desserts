...
export default async function AdminHome() {
  ...
  return (<><h1 className="admin-h1">Overview</h1><p className="admin-sub">Manage your storefront</p><div className="admin-grid" style={{marginBottom:"1.6rem"}}>{[{products??0,"Total products"},{activeProducts??0,"Active products"},{categories??0,"Categories"},{enquiries??0,"Enquiries"},{newEnquiries??0,"New enquiries"}].map(([b,s])=>(<div className="stat" key={s}><b>{b}</b><span>{s}</span></div>))}</div><div className="admin-card"><h2 style={{fontFamily:"var(--font-d)",fontSize:"1.2rem",color:"var(--cream)",marginBottom:".8rem"}}>Quick actions</h2><div className="row-actions"><Link className="btn btn-gold" href="/admin/products">Manage products</Link><Link className="btn btn-ghost" href="/admin/categories">Manage categories</Link><Link className="btn btn-ghost" href="/admin/enquiries">View enquiries</Link></div></div></>);
}
